...
{
    my $self = shift;
    my $dbh = DBI->connect($self->{db}, $self->{user}, $self->{pass}, {AutoCommit => 0, PrintError => 1,RaiseError => 1, LongReadLen => 36000000, }) or die "Couldn't connect to database: " . DBI->errstr;
    $self->{dbh} = $dbh;
